...
In this folder, you will see example.py and example_crabConfig.py. These are both taken from a ScoutingNano. I want to show you what needs to change in test.py and what a simple crab configuration file looks like. But before we look at that, we should make sure crab is set up properly.
...
If you ever get a `crab not found` style error, run the `source` line again. Also, you should make your singularity either el8 or el9, depending on your CMSSW release.
...
In example.py, the only thing you need to change is how you are pointing the script to your file. You give crab a dataset, and crab will then run the whole dataset. Since crab imports chunks of that dataset per job, you want to tell this script to look for a string that crab gives it.  The commented section is what you would normally have when you run the script locally, where you implicitly specify the root file you want to run on.
...
Now for the crab configuration script! Now I am going to highlight each important section
...
This next section specifies which script to run and whether anything special needs to happen.
...
This section provides Crab with all the specifics about the job. Like, what dataset are you using, how are you splitting the data up, and based on that splitting, how large should each job be? Then, finally, what it should do with the output, if it should publish it to DAS, if it should have a tag, and where to store it. Once the job is done and saved, the T3 you can find your files under `/cms/cephfs/data/store/user/<cern username>/`.
...
crab checkwrite --site=T3_US_NotreDame
```
Why we have to do the crab setup twice, I have no clue, but this works. We need to do this because this time we are submitting using Python rather than `crab submit`. This is done so that we just have a list of all of our mass hypotheses, and we don't need to submit 20 to 30 unique crab jobs.
...
This makes the LHE step run on only one thread; crab can't do multithreaded processing like this, so it is safer to do it this way. And finally,
...
Now we can look at the config file. As I have said before, submitting 20+ crab jobs is not what we want to do, so let's take a look at GEN_SIM_crabConfig.py in more detail.
...
```
This function defines all of the crab configuration we need.
...
cfg.Data.totalUnits              = 2000         # 20 jobs x 100 events
```
This part tells it to split my jobs based on the number of events: each job should handle 100 events, and there should be 2000 events in total. Now, finally, the last part
...
This is unfortunately not a bulletproof guide, but LLMs are good, and I have seen many issues before, so please reach out to your favorite AI or me, and I can hopefully get everything fixed!
